...
<div class="kt-portlet kt-portlet--mobile">
    ...
    <div class="kt-portlet__body">
        ...
        <table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
            <thead>
                <tr>
                    <th width="200px">ID Kategori Acara</th>
            ...
            <tbody>
                ...
                $sql="SELECT id_kategori_event, kategori_event, desc_kategori FROM `tb_kategori_event` WHERE status_kat_event=0";
    ...
    <div class="kt-portlet__body">
        ...
        <table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_2">
            ...
            <tbody>
                ...
                $sql="SELECT tb_req_kat_event.id_req_kat_event, tb_req_kat_event.req_kat_event, GROUP_CONCAT(tb_kategori_event.kategori_event) AS kategori_event, tb_req_kat_event.id_event, tb_event.nama_event,tb_event.desc_event,tb_event.status_publish, DATE_FORMAT(tb_req_kat_event.created_at, '%d/%m/%Y') AS created_at, tb_req_kat_event.status_req_kat_event FROM `tb_req_kat_event`, tb_event, tb_kategori_event, tb_det_kategori_event WHERE tb_req_kat_event.id_event = tb_event.id_event AND tb_event.id_event=tb_det_kategori_event.id_event AND tb_kategori_event.id_kategori_event=tb_det_kategori_event.id_kategori_event GROUP BY tb_event.id_event ORDER BY tb_req_kat_event.status_req_kat_event";
                $result=mysqli_query($connect,$sql);
                $no = 1;
                while($row=mysqli_fetch_assoc($result)){
                    ...
                    <tr>
                    <td>'.$no.'</td>
                    <td>'.$row['req_kat_event'].'</td>
                    <td>'.$row['kategori_event'].'</td>
                    <td><a href="" data-toggle="modal" data-target="#modal_change_kategori" onclick="setId(\''.$row['id_event'].'\',\''.$row['desc_event'].'\',\''.$row['status_publish'].'\')">'.$row['nama_event'].'</a></td>
                    
                    <td>'.$row['created_at'].'</td>
                    <td nowrap>
                        '. ($row['status_req_kat_event'] == '1' ? '
                        <span class="kt-badge  kt-badge--success kt-badge--inline kt-badge--pill">Sudah ditambahkan</span>' :
                            ($row['status_req_kat_event'] == '2' ? '
                            <span class="kt-badge  kt-badge--dark kt-badge--inline kt-badge--pill">Dihapus</span>' :
                            ($row['status_req_kat_event'] == '0' ? '
                            <a href="#form_tambah_data" onclick="showKategori(\''.$row['id_req_kat_event'].'\',\''.$row['req_kat_event'].'\')" class="btn btn-sm btn-clean btn-icon-md" title="Tambah">
                            <i class="la la-plus-circle"></i> 
                            </a> 
                            <a href="" class="btn btn-sm btn-clean btn-icon-md" onclick="confirmSubmitDelete(\'halaman/delete_kategori_event_lainnya.php?id='.$row['id_req_kat_event'].'\')" data-toggle="modal" data-target="#modal_submit_delete" id="btn_delete" title="Hapus">
                            <i class="la la-trash"></i>
                            </a>' : 
                            ''))).'
                    </td>
